...
class ASGCN_BERT(nn.Module):
    def __init__(self, bert, opt, datautil):
    ...
    def forward(self, inputs):
        concat_bert_indices, concat_segments_indices, aspect_indices, left_indices, adj = inputs
        if self.datautil == True:
            concat_bert_indices = torch.squeeze(concat_bert_indices, dim=1)
            concat_segments_indices = torch.squeeze(concat_segments_indices, dim=1)
        text_len = torch.sum(concat_bert_indices != 0, dim=-1)
        aspect_len = torch.sum(aspect_indices != 0, dim=-1)
        left_len = torch.sum(left_indices != 0, dim=-1)
        aspect_double_idx = torch.cat([left_len.unsqueeze(1), (left_len+aspect_len-1).unsqueeze(1)], dim=1)
        text, _ = self.bert(concat_bert_indices, concat_segments_indices)
        text_out = self.dropout(text)
